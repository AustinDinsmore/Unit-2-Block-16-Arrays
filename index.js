...
function calculateTotalInventory() {
  return items.reduce((invTotal, item) => invTotal + item.inventory, 0)
...
function calculateAllInventoryPrice() {
  return items.reduce((grandTotal, item) => grandTotal + (item.price * item.inventory), 0)
...
function getItemPriceByName(name) {
  let itemSearched = items.find(item => item.name === name)
  return itemSearched.price 
...
function logCartItems() {
  for(let i = 0; i < cart.length; i++) {
    console.log(findItemById(cart[i]) .name)
  }
...
function calculateTotalCartPrice() {
  ...
  return cart.reduce((total, itemId) => total + findItemById(itemId).price,0.0)
}
